        ...
        NavigationView {
            Form {
                ...
                Section{
                    ...
                    Text("有些 App 會指定自家開發者帳號的鑰匙圈群組，在本程式中執行時必然無權存取，導致取不到金鑰而無法解密自己的資料（例如新版 LINE 登入後重開就失效）。開啟後會移除這類指定，改用系統配給本程式的群組，並在項目名稱加註容器識別碼，使各容器仍維持各自獨立的登入狀態。\n\n只處理指定了其他開發者帳號的 App，原本就能正常運作的不受影響。切換此選項會改變項目的存放位置，受影響的 App 需要重新登入。")
                        .font(.footnote)
                        .foregroundStyle(.gray)
